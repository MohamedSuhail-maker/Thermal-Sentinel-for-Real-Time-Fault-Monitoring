    ...
    try
        ...
        if tcpObj.BytesAvailable > 0
            ...
            if numel(thermalData) == 64
                ...
                % Send Email Alert if fault is detected and cooldown time has passed
                if faultDetected && (currentTime - emailSentTime > emailCooldown)
                    sendmail('recipient_email@example.com', '⚠ Fault Detected! HIGH TEMPERATURE ', ...
                        'High-temperature hotspot detected. Please check the system.');
